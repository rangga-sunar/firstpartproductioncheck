...
class C212_controller extends CI_Controller
{
    ...
    {
        $list = $this->C212_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->C212_model->count_all(),
            "recordsFiltered" => $this->C212_model->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data = $this->C212_model->get_by_id($id);
    ...
    public function ajax_add()
    {
        ...
        if ($this->input->post('partnumber') === '') {
            ...
            $insert = $this->C212_model->save($data);
    ...
    public function ajax_update()
    {
        ...
        $this->C212_model->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }

    public function ajax_delete($id)
    {
        $this->C212_model->delete_by_id($id);
        echo json_encode(array("status" => TRUE));
    ...
    public function do_upload()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $result = $this->C212_model->upload_file($data, $where);
    ...
    public function upload_file()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $this->C212_model->upload_file($data, $where);

            redirect('C212_controller');
        } else {
