...
class CN235_controller extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('nik') != '') {
            ...
            $this->load->view('user/view_CN235');
    ...
    {
        $list = $this->CN235_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->CN235_model->count_all(),
            "recordsFiltered" => $this->CN235_model->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data = $this->CN235_model->get_by_id($id);
    ...
    public function ajax_add()
    {
        ...
        if ($this->input->post('partnumber') === '') {
            ...
            $insert = $this->CN235_model->save($data);
    ...
    {
        $this->CN235_model->delete_by_id($id);
        echo json_encode(array("status" => TRUE));
    ...
    public function do_upload()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $result = $this->CN235_model->upload_file($data, $where);
    ...
    public function upload_file()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $this->CN235_model->upload_file($data, $where);

            redirect('CN235_controller');
        } else {
