...
class KFX_controller extends CI_Controller
{
    ...
    {
        $list = $this->KFX_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $program = substr($this->uri->segment(1), 0, 3);
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->KFX_model->count_all(),
            "recordsFiltered" => $this->KFX_model->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data = $this->KFX_model->get_by_id($id);
    ...
    public function ajax_add()
    {
        ...
        if ($this->input->post('partnumber') === '') {
            ...
            $insert = $this->KFX_model->save($data);
    ...
    public function ajax_update()
    {
        ...
        $this->KFX_model->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }

    public function ajax_delete($id)
    {
        $this->KFX_model->delete_by_id($id);
        echo json_encode(array("status" => TRUE));
    ...
    public function do_upload()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $result = $this->KFX_model->upload_file($data, $where);
    ...
    public function upload_file()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $this->KFX_model->upload_file($data, $where);

            redirect('KFX_controller');
        } else {
