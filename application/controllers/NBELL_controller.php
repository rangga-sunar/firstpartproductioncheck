...
class NBELL_controller extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('nik') != '') {
            ...
            $this->load->view('user/view_NBELL');
    ...
    {
        $list = $this->NBELL_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $program = substr($this->uri->segment(1), 0, 5);
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->NBELL_model->count_all(),
            "recordsFiltered" => $this->NBELL_model->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data = $this->NBELL_model->get_by_id($id);
    ...
    public function ajax_add()
    {
        ...
        if ($this->input->post('partnumber') === '') {
            ...
            $insert = $this->NBELL_model->save($data);
    ...
    public function ajax_update()
    {
        ...
        $this->NBELL_model->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }

    public function ajax_delete($id)
    {
        $this->NBELL_model->delete_by_id($id);
        echo json_encode(array("status" => TRUE));
    ...
    public function do_upload()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $result = $this->NBELL_model->upload_file($data, $where);
    ...
    public function upload_file()
    {
        ...
        if ($this->upload->do_upload('filename')) {
            ...
            $where = array('id' => $id);
            $this->NBELL_model->upload_file($data, $where);

            redirect('NBELL_controller');
        } else {
